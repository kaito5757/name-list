  ...
  main_image: z
    ...
    .refine(
      (file) => {
        ...
        if (file instanceof FileList) {
          return file.length !== 0 && file[0]?.size < 500000;
        }
      },
    ...
    .refine(
      (file) => {
        if (file instanceof File) {
          return file?.type !== "" || IMAGE_TYPES.includes(file?.type);
        }
        if (file instanceof FileList) {
          return file.length !== 0 && IMAGE_TYPES.includes(file[0].type);
        }
      },
      { message: "拡張子が「jpg、png」の画像を選択してください。" },
    ),
  full_name: z
    .string()
